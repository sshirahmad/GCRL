...
def main(args):
    ...
    sigma = torch.nn.Parameter(torch.tensor(-2.0, device="cuda"))
    ...
    training_steps = {f'P{i}': [sum(args.num_epochs[:i - 1]), sum(args.num_epochs[:i])] for i in range(1, 5)}
        ...
        for batch_idx in range(train_dataset['num_batches']):
            ...
            for train_idx, (env_iter, env_name) in enumerate(zip(train_iter, train_dataset['names'])):
                ...
                if training_step in ["P1", "P2"]:
                    ...
                    single_env_loss = (- predict_loss) + torch.exp(sigma) * (- elbo_loss) + (- theta_loss) + (- theta_constraint)
